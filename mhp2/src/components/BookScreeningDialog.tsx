...
const BookScreeningDialog = ({ open, onOpenChange, onBookingSuccess }: BookScreeningDialogProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Calculate age from DOB for the child_age field
      const dobDate = new Date(form.child_dob);
      const ageDiff = Date.now() - dobDate.getTime();
      const ageYears = Math.floor(ageDiff / (1000 * 60 * 60 * 24 * 365.25));
      const childAge = isNaN(ageYears) ? form.child_dob : `${ageYears} years`;

      const { error } = await supabase.from("parent_bookings").insert({
        parent_name: form.parent_name,
        child_name: form.child_name,
        child_age: childAge,
        gender: form.gender,
        phone: form.phone,
        email: `${form.phone}@mhp.in`,   // email NOT NULL — using phone as placeholder since form has no email field
        preferred_date: form.preferred_date ? `${form.preferred_date} ${form.preferred_time}` : null,
      });
      if (error) {
        console.error("Booking error:", error.message, error.details, error.code);
