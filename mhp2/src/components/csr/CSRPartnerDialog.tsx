  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("school_leads").insert({
        full_name: form.name,
        school_name: form.organization,
        email: form.email || "",
        phone: form.phone,
        program_interest: `CSR: ${form.partnership_type}${form.message ? ` — ${form.message}` : ""}`,
      });
      if (error) {
        console.error("CSR partner error:", error.message, error.details, error.code);
