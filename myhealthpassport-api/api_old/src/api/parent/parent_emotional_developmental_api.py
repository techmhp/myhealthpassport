...
#         #         question_id=idx,
#         #         question_text=q["question_text"],
#         #         question_type="Developmental & Emotional",
#         #         sub_domain=q.get("sub_domain") or None,
#         #         options=["Yes", "No"],
#         #         score_type=q["score_type"],
#         #         answer=None
...
#             if q.get("applicable_to_parent", False) and is_grade_in_range(class_room, q["grade_level"]):
#                 questions.append(QuestionResponse(
#                     question_id=ParentQuestionID.EMOTIONAL_START + idx,  # ← Unique!
#                     question_text=q["question_text"],
#                     question_type="EMOTIONAL_DEVELOPMENTAL",
#                     sub_domain=q.get("sub_domain") or None,
#                     options=["Yes", "No"],
...
#         ).order_by('-updated_at').values("question_id", "answer", "notes")
#         answer_map = {answer["question_id"]: answer["answer"] for answer in existing_answers}
#         # Get the most recent non-empty note and strip the prefix
#         notes = None
#         for answer in existing_answers:
#             if answer["notes"] and answer["notes"].startswith("Developmental & Emotional:"):
#                 notes = answer["notes"].replace("Developmental & Emotional:", "").strip()
#                 break

#         # for question in questions:
#         #     question.answer = answer_map.get(question.question_id)
#         # Convert numeric string answers to integers
#         for question in questions:
#             string_answer = answer_map.get(question.question_id)
#             if string_answer is not None and string_answer.isdigit():
#                 question.answer = int(string_answer)  # Convert numeric string to integer
#             else:
#                 question.answer = string_answer  # Keep non-numeric strings or None as-is
...
#         class_room = normalize_grade(student.class_room)
#         # valid_questions = {
#         #     idx: q for idx, q in enumerate(PARENT_EMOTIONAL)
#         #     if is_grade_in_range(class_room, q["grade_level"]) and q.get("applicable_to_parent", False)
#         # }
#         valid_questions = {
#             ParentQuestionID.EMOTIONAL_START + idx: q
#             for idx, q in enumerate(PARENT_EMOTIONAL)
#             if is_grade_in_range(class_room, q["grade_level"]) and q.get("applicable_to_parent", False)
#         }
#         # invalid_question_ids = [a.question_id for a in batch.answers if a.question_id not in valid_questions]
#         invalid_question_ids = [
#                 a.question_id for a in batch.answers
#                 if a.question_id not in valid_questions
#             ]
#         if invalid_question_ids:
#             response_obj = StandardResponse(
#                 status=False,
#                 message="One or more question IDs are invalid for the student's class room",
#                 errors={"detail": f"Invalid question IDs: {invalid_question_ids}"}
#             )
#             return JSONResponse(content=response_obj.__dict__, status_code=status.HTTP_400_BAD_REQUEST)

#         # Prefix notes with question type
#         prefixed_notes = f"Developmental & Emotional: {batch.notes or ''}".strip()
...
#                     response_obj = StandardResponse(
#                         status=False,
#                         message=f"Question ID {answer.question_id} not found",
#                         errors={"detail": "Invalid question ID"}
#                     )
#                     return JSONResponse(content=response_obj.__dict__, status_code=status.HTTP_400_BAD_REQUEST)

#                 existing_answer = await ParentAnswers.filter(
#                     student_id=answer.student_id,
#                     question_id=answer.question_id
#                 ).first()
...
#                 notes=batch.notes  # Return original notes without prefix
#             ).dict()
#         )
#         return JSONResponse(content=response_obj.__dict__, status_code=status.HTTP_200_OK)
