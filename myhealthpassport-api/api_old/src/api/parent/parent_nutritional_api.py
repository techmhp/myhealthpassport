...
#     NUTRITIONAL_HARDCODED_START = 10000
#     EMOTIONAL_HARDCODED_START   = 20000
...
# # questions from questions list and db
# @router.get("/parent-nutritional-questions/{student_id}", response_model=StandardResponse)
# async def get_nutritional_questions(
#     student_id: int,
#     current_user: Any = Depends(get_current_authorized_user)
...
#             ).first()
#             if not parent_child:
#                 valid_students = await ParentChildren.filter(parent_id=current_user.id, status=True).values_list("student_id", flat=True)
#                 response_obj = StandardResponse(
#                     status=False,
...
#         questions = []
#         seen_question_texts = set()  # To avoid duplicates
...
#         # 2. Add questions from HARDCODED ARRAY (avoiding duplicates)
#         max_db_id = max([q.question_id for q in questions], default=0)
...
#                 if question_text_normalized not in seen_question_texts:
#                     questions.append(QuestionResponse(
#                         # question_id=max_db_id + idx + 1,  # Use unique ID higher than DB IDs
#                         # question_id=ParentQuestionID.NUTRITIONAL_START + idx,
#                         question_id=ParentQuestionID.NUTRITIONAL_HARDCODED_START + idx,
#                         question_text=q["question_text"],
#                         question_type="NUTRITIONAL",
#                         sub_domain=q.get("sub_domain", ""),
#                         options=["Yes", "No"],
...
#             response_obj = StandardResponse(
#                 status=False,
#                 message=f"No questions available for this class {class_room}",
#                 errors={"detail": f"Class {class_room} has no available nutritional questions"}
#             )
#             return JSONResponse(content=response_obj.__dict__, status_code=status.HTTP_400_BAD_REQUEST)

#         # Get existing answers for ALL questions (both DB and hardcoded)
#         all_question_ids = {q.question_id for q in questions}
#         existing_answers = await ParentAnswers.filter(
#             student_id=student_id,
#             question_id__in=all_question_ids,
...
#         # Get the most recent non-empty note
#         notes = None
#         for answer in existing_answers:
#             if answer["notes"] and answer["notes"].startswith("NUTRITIONAL:"):
#                 notes = answer["notes"].replace("NUTRITIONAL:", "").strip()
#                 break

#         # Set answers and calculate health score
#         for question in questions:
#             string_answer = answer_map.get(question.question_id)
#             if string_answer is not None and string_answer.isdigit():
#                 question.answer = int(string_answer)
...
#         # Count questions from each source for message
#         db_count = len([q for q in questions if q.question_id <= max_db_id])
#         hardcoded_count = len([q for q in questions if q.question_id > max_db_id])

#         response_obj = StandardResponse(
#             status=True,
#             message=f"Parent nutritional questions retrieved successfully. Found {len(questions)} questions ({db_count} from database, {hardcoded_count} from hardcoded)",
#             data=QuestionListResponse(
#                 questions=questions,
#                 student=student_info,
#                 notes=notes,
...
# # === REPLACE THE ENTIRE submit-parent-nutritional-answers function with this ===
# @router.post("/submit-parent-nutritional-answers", response_model=StandardResponse)
# async def submit_nutritional_answers(
#     batch: AnswerBatchRequest,
#     current_parent: Any = Depends(get_current_parent)
...
#         student_ids = {answer.student_id for answer in batch.answers}
#         if len(student_ids) > 1:
#             return StandardResponse(status=False, message="All answers must belong to the same student", errors={"detail": "Multiple student IDs"})
...
#             return StandardResponse(status=False, message=f"Unauthorized for student {student_id}", errors={"valid_students": list(valid_students or [])})

#         class_room = normalize_grade(student.class_room)

#         # === CRITICAL FIX: Use same ID logic as GET endpoint ===
#         valid_questions = []
#         db_questions = await StudentsQuestionBank.filter(
#             question_type="NUTRITIONAL",
#             applicable_to_parent=True,
...
#         # DB questions: use real question_id
#         for db_question in db_questions:
#             question_grades = [g.strip() for g in db_question.grade_level.split(",")]
#             normalized_grades = [normalize_grade(g) for g in question_grades]
#             if is_grade_in_range(class_room, normalized_grades):
...
#         # Hardcoded questions: use offset ID (10000 + idx)
#         for idx, q in enumerate(PARENT_NUTRITIONAL):
#             if is_grade_in_range(class_room, q["grade_level"]):
#                 hardcoded_id = ParentQuestionID.NUTRITIONAL_HARDCODED_START + idx
#                 question_text = q["question_text"].lower().strip()
#                 # Avoid duplicate with DB
#                 if not any(qq["question_text"].lower().strip() == question_text for qq in valid_questions):
#                     valid_questions.append({
#                         "question_id": hardcoded_id,
#                         "question_text": q["question_text"],
...
#                 status=False,
#                 message="Invalid question IDs for this student's grade",
#                 errors={"invalid_question_ids": invalid_ids, "valid_range": f"DB:1+, Hardcoded:{ParentQuestionID.NUTRITIONAL_HARDCODED_START}+"} 
#             )
...
#                 existing = await ParentAnswers.filter(
#                     student_id=student_id,
#                     question_id=answer.question_id
#                 ).first()
