...
## this is for at least one question answered or not
...
#     student_list = []
#     # Single DB query for assign_school
#     assign_school = await AssignSchool.filter(school=school_id, is_deleted=False).first()
...
#             # ===============================================
#             # CHECK AVAILABLE QUESTIONS FOR THIS CLASS
#             # ===============================================
            
#             # --- PARENT QUESTIONS AVAILABILITY ---
#             parent_nutritional_available = False
#             parent_emotional_available = False
            
#             # Check DB questions for parents
#             db_parent_questions = await StudentsQuestionBank.filter(
#                 applicable_to_parent=True,
#                 is_deleted=False
#             ).all()
            
#             for dbq in db_parent_questions:
...
#                     if dbq.question_type == "NUTRITIONAL":
#                         parent_nutritional_available = True
#                     elif dbq.question_type == "EMOTIONAL_DEVELOPMENTAL":
#                         parent_emotional_available = True
            
#             # Check hardcoded parent questions
#             if not parent_nutritional_available:
#                 from src.api.parent.nutritional_questions_parents import NUTRITIONAL_QUESTIONS as PARENT_NUTRITIONAL
#                 for q in PARENT_NUTRITIONAL:
#                     if is_grade_in_range(class_room, q.get("grade_level", [])):
#                         parent_nutritional_available = True
#                         break
            
#             if not parent_emotional_available:
...
#                 for q in PARENT_EMOTIONAL:
#                     if is_grade_in_range(class_room, q.get("grade_level", [])):
#                         parent_emotional_available = True
#                         break
            
#             # --- TEACHER QUESTIONS AVAILABILITY ---
#             teacher_nutritional_available = False
#             teacher_emotional_available = False
            
#             # Check DB questions for teachers
#             db_teacher_questions = await StudentsQuestionBank.filter(
#                 applicable_to_teacher=True,
#                 is_deleted=False
#             ).all()
            
#             for dbq in db_teacher_questions:
#                 grades = [g.strip() for g in dbq.grade_level.split(",") if g.strip()]
#                 if is_grade_in_range(class_room, [normalize_grade(g) for g in grades]):
#                     if dbq.question_type == "NUTRITIONAL":
#                         teacher_nutritional_available = True
#                     elif dbq.question_type == "EMOTIONAL_DEVELOPMENTAL":
#                         teacher_emotional_available = True
            
#             # Check hardcoded teacher questions
#             if not teacher_nutritional_available:
#                 from src.api.teacher.nutritional_questions_teachers import NUTRITIONAL_QUESTIONS as TEACHER_NUTRITIONAL
#                 for q in TEACHER_NUTRITIONAL:
#                     if is_grade_in_range(class_room, q.get("grade_level", [])) and q.get("applicable_to_teacher", False):
#                         teacher_nutritional_available = True
#                         break
            
#             if not teacher_emotional_available:
...
#             # ===============================================
#             parent_answers = await ParentAnswers.filter(
...
#             # ===============================================
#             # PARENT QUESTIONNAIRE STATUS CHECK
#             # ===============================================
#             parent_nutritional_answered = False
#             parent_emotional_answered = False
            
#             for ans in parent_answers:
#                 qid = ans.question_id
                
#                 # Parent Nutritional: >= 1,000,000 AND < 2,100,000
#                 if 1000000 <= qid < 2100000:
#                     parent_nutritional_answered = True
#                 # Parent Emotional: >= 2,100,000 AND < 3,100,000
#                 elif 2100000 <= qid < 3100000:
#                     parent_emotional_answered = True
#                 # Check DB questions (< 1,000,000)
#                 elif qid < 1000000:
#                     db_q = await StudentsQuestionBank.get_or_none(
#                         question_id=qid,
#                         applicable_to_parent=True,
#                         is_deleted=False
#                     )
#                     if db_q:
#                         if db_q.question_type == "NUTRITIONAL":
#                             parent_nutritional_answered = True
#                         elif db_q.question_type == "EMOTIONAL_DEVELOPMENTAL":
#                             parent_emotional_answered = True
                
#                 # Early exit if both found
#                 if parent_nutritional_answered and parent_emotional_answered:
#                     break
            
#             # LOGIC: Status is TRUE if all available question types are answered
#             if parent_nutritional_available and parent_emotional_available:
#                 # Both types available - both must be answered
#                 parent_questionnaire_status = parent_nutritional_answered and parent_emotional_answered
#             elif parent_nutritional_available and not parent_emotional_available:
#                 # Only nutritional available
#                 parent_questionnaire_status = parent_nutritional_answered
#             elif parent_emotional_available and not parent_nutritional_available:
#                 # Only emotional available
#                 parent_questionnaire_status = parent_emotional_answered
#             else:
#                 # No questions available for this class
#                 parent_questionnaire_status = True

#             # ===============================================
#             # TEACHER QUESTIONNAIRE STATUS CHECK
#             # ===============================================
#             teacher_nutritional_answered = False
#             teacher_emotional_answered = False
            
#             for ans in teacher_answers:
#                 qid = ans.question_id
                
#                 # Teacher Nutritional: >= 3,100,000 AND < 4,100,000
#                 if 3100000 <= qid < 4100000:
#                     teacher_nutritional_answered = True
#                 # Teacher Emotional: >= 4,100,000 AND < 5,100,000
#                 elif 4100000 <= qid < 5100000:
#                     teacher_emotional_answered = True
#                 # Check DB questions (< 1,000,000)
#                 elif qid < 1000000:
#                     db_q = await StudentsQuestionBank.get_or_none(
#                         question_id=qid,
...
#                 # Early exit if both found
#                 if teacher_nutritional_answered and teacher_emotional_answered:
#                     break
            
#             # LOGIC: Same as parent
#             if teacher_nutritional_available and teacher_emotional_available:
#                 # Both types available - both must be answered
#                 teacher_questionnaire_status = teacher_nutritional_answered and teacher_emotional_answered
#             elif teacher_nutritional_available and not teacher_emotional_available:
#                 # Only nutritional available
#                 teacher_questionnaire_status = teacher_nutritional_answered
#             elif teacher_emotional_available and not teacher_nutritional_available:
#                 # Only emotional available
#                 teacher_questionnaire_status = teacher_emotional_answered
#             else:
#                 # No questions available for this class
#                 teacher_questionnaire_status = True

#             # Add questionnaire statuses to student data
...
#     # Add screening statuses for OnGroundTeamRoles and ScreeningTeamRoles
#     role_status_map = {
...
#         # Batch queries for all students
...
#             # Check if student is in students
...
#             # Update all statuses dynamically for OnGroundTeamRoles
...
#                 # Compute completed_status: True only if all statuses are True
...
#                 # Get medical screening statuses for eye specialist
...
#                 # Get medical screening statuses for dentist
#                 if student_id in medical_status_lookup:
...
#                 # Get medical screening statuses for this nutritionist
#                 if student_id in medical_status_lookup:
...
#                 # Get medical screening statuses for this psychologist
#                 if student_id in medical_status_lookup:
