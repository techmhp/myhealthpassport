...
#     if role_to_check != "MEDICAL_OFFICER":
#         response_obj = StandardResponse(
#             status=False,
#             message="Only medical officers can access this endpoint",
#             errors={"detail": f"Invalid user_role: {role_to_check}. Required role: MEDICAL_OFFICER"}
#         )
#         return JSONResponse(content=response_obj.__dict__, status_code=status.HTTP_403_FORBIDDEN)

#     medical_officer = await AnalystTeam.get_or_none(id=user["user_id"], user_role__iexact=role_to_check)
#     if not medical_officer:
#         response_obj = StandardResponse(
#             status=False,
#             message="Medical officer not found",
#             errors={"detail": f"No medical officer found for user_id: {user['user_id']}"}
#         )
