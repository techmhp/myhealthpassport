...
#         # 1. Database Questions (IDs: 1–99999)
#         db_qs = await StudentsQuestionBank.filter(
#             question_type="EMOTIONAL_DEVELOPMENTAL",
#             applicable_to_teacher=True,
#             is_deleted=False
...
#                     question_type="EMOTIONAL_DEVELOPMENTAL",
#                     sub_domain=dbq.sub_domain or "",
#                     options=["1", "2", "3"],           # ← Now 1,2,3
#                     score_type=dbq.score_type,
#                     answer=None
#                 ))
#                 seen_texts.add(dbq.question_text.lower().strip())

#         # 2. Hardcoded Questions → IDs 100000+
#         for idx, q in enumerate(TEACHER_EMOTIONAL):
#             q_grades = [normalize_grade(g) for g in q["grade_level"]]
#             if (is_grade_in_range(class_room, q_grades) and q.get("applicable_to_teacher", False)):
#                 txt = q["question_text"].lower().strip()
#                 if txt not in seen_texts:
#                     qid = TeacherQuestionID.EMOTIONAL_HARDCODED_START + idx
#                     questions.append(QuestionResponse(
#                         question_id=qid,
#                         question_text=q["question_text"],
#                         question_type="EMOTIONAL_DEVELOPMENTAL",
#                         sub_domain=q.get("sub_domain", ""),
#                         options=["1", "2", "3"],       # ← 1,2,3 only
#                         score_type=q["score_type"],
#                         answer=None
#                     ))
#                     seen_texts.add(txt)
...
#         # Load saved answers (stored as "1", "2", "3" strings)
#         saved_answers = await TeacherAnswers.filter(
#             student_id=student_id,
#             question_id__in=[q.question_id for q in questions],
#             is_deleted=False
...
#         # Attach saved answer (already "1", "2", "3" or null)
#         for q in questions:
#             saved = answer_map.get(q.question_id)
#             if saved in ["1", "2", "3"]:
#                 q.answer = saved  # ← Already string "1", "2", or "3"
#             # else remains None
...
#     current_teacher: Any = Depends(get_current_teacher)
# ):
#     # If auth dependency returned an error response already
#     if isinstance(current_teacher, JSONResponse):
#         return current_teacher

#     try:
#         # 1. Must be same student
#         student_ids = {a.student_id for a in batch.answers}
#         if len(student_ids) != 1:
#             return JSONResponse(
#                 content=StandardResponse(
#                     status=False,
#                     message="All answers must belong to the same student",
#                     errors={"detail": "Multiple student_ids detected"}
#                 ).__dict__,
...
#         if not student:
#             return JSONResponse(
#                 content=StandardResponse(status=False, message="Student not found").__dict__,
#                 status_code=status.HTTP_404_NOT_FOUND,
#             )

#         # 3. Teacher is authorized for this student
#         authorized = await SchoolStudents.filter(
#             student_id=student_id,
...
#         if not authorized:
#             return JSONResponse(
#                 content=StandardResponse(status=False, message="You are not authorized for this student").__dict__,
...
#         # ─────────────────────────────────────────────────────────────
#         # Build the exact same question list that the GET endpoint returns
#         # ─────────────────────────────────────────────────────────────
#         valid_questions: dict[int, dict] = {}
#         seen_texts = set()

#         # 1. DB questions (grade_level is a comma-separated string)
#         db_qs = await StudentsQuestionBank.filter(
#             question_type="EMOTIONAL_DEVELOPMENTAL",
#             applicable_to_teacher=True,
#             is_deleted=False,
...
#         # 2. Hardcoded questions – use stable IDs 100000+
#         for idx, q in enumerate(TEACHER_EMOTIONAL):
#             # q["grade_level"] is a LIST → normalize each entry
#             q_grades = [normalize_grade(g) for g in q["grade_level"]]
#             if (
#                 is_grade_in_range(class_room, q_grades)
#                 and q.get("applicable_to_teacher", False)
#             ):
#                 txt = q["question_text"].lower().strip()
#                 if txt not in seen_texts:
#                     stable_id = TeacherQuestionID.EMOTIONAL_HARDCODED_START + idx
#                     valid_questions[stable_id] = {
#                         "text": q["question_text"],
#                         "score_type": q["score_type"],
#                     }
...
#         # ─────────────────────────────────────────────────────────────
#         # Validate incoming question_ids
#         # ─────────────────────────────────────────────────────────────
#         invalid_ids = [a.question_id for a in batch.answers if a.question_id not in valid_questions]
#         if invalid_ids:
#             return JSONResponse(
#                 content=StandardResponse(
#                     status=False,
#                     message="Invalid question IDs for this grade",
#                     errors={
#                         "invalid_ids": invalid_ids,
#                         "valid_sample": list(valid_questions.keys())[:15],
#                         "student_grade": class_room,
#                     },
#                 ).__dict__,
#                 status_code=status.HTTP_400_BAD_REQUEST,
#             )
...
#             content=StandardResponse(
#                 status=True,
#                 message="Teacher emotional & developmental answers submitted successfully",
#                 data=AnswerListResponse(answers=saved_answers, notes=batch.notes or "").dict(),
#             ).__dict__,
...
#             content=StandardResponse(
#                 status=False,
#                 message="Server error while saving emotional answers",
