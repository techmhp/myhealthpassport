...
# ──────────────────────────────────────────────────────────────
#  GLOBAL TEACHER QUESTION ID RANGES (Add this at top of file)
# ──────────────────────────────────────────────────────────────
...
#             ).__dict__, status_code=404)

#         if isinstance(current_user, SchoolStaff):
#             if not await SchoolStudents.filter(
#                 student_id=student_id, school_id=current_user.school_id, status=True
#             ).exists():
#                 return JSONResponse(content=StandardResponse(
...
#         # 1. Database Questions (real IDs: 1–99999)
#         db_questions = await StudentsQuestionBank.filter(
#             question_type="NUTRITIONAL",
#             applicable_to_teacher=True,
#             is_deleted=False
...
#         # 2. Hardcoded Nutritional Questions → Use 200000+
#         for idx, q in enumerate(TEACHER_NUTRITIONAL):
#             if (is_grade_in_range(class_room, q["grade_level"]) and 
#                 q.get("applicable_to_teacher", False)):
#                 txt = q["question_text"].lower().strip()
#                 if txt not in seen_texts:
#                     question_id = TeacherQuestionID.NUTRITIONAL_HARDCODED_START + idx
#                     questions.append(QuestionResponse(
#                         question_id=question_id,
#                         question_text=q["question_text"],
#                         question_type="NUTRITIONAL",
...
#         if not questions:
#             return JSONResponse(content=StandardResponse(
#                 status=False, message=f"No nutritional questions for class {class_room}"
#             ).__dict__, status_code=400)
...
#         answer_map = {a["question_id"]: a["answer"] for a in existing}
#         notes = next((a["notes"].replace("NUTRITIONAL:", "").strip() 
#                      for a in existing if a["notes"] and a["notes"].startswith("NUTRITIONAL:")), None)
...
#     except Exception as e:
#         return JSONResponse(content=StandardResponse(
#             status=False, message=str(e), errors={"detail": str(e)}
...
#         # Hardcoded Questions – FIX: grade_level is LIST
#         for idx, q in enumerate(TEACHER_NUTRITIONAL):
#             q_grades = [normalize_grade(g) for g in q["grade_level"]]
#             if (is_grade_in_range(class_room, q_grades) and q.get("applicable_to_teacher", False)):
#                 txt = q["question_text"].lower().strip()
#                 if txt not in seen_texts:
#                     qid = TeacherQuestionID.NUTRITIONAL_HARDCODED_START + idx
#                     valid_questions[qid] = {"text": q["question_text"], "score_type": q["score_type"]}
#                     seen_texts.add(txt)
...
#         # Rest of saving logic (unchanged)
#         prefixed_notes = f"NUTRITIONAL: {batch.notes or ''}".strip()
#         saved = []
#         async with in_transaction():
#             for a in batch.answers:
