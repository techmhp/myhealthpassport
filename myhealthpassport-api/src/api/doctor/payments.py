    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "slot_date": "2025-10-10",   # Already string
                "slot_time": "10:30",
                "consult_fee": 500.0,
...
# ===================================================================
# MODIFIED: GET APPOINTMENTS BY STUDENT (with Academic Year Filter)
# ===================================================================
@router.get("/appointments-bystudent/{student_id}", response_model=StandardResponse)
async def get_appointments_by_student(
    student_id: int,
    ...
    # Build academic year filter - use created_at only for both models
    year_filter = Q(created_at__gte=ay_start, created_at__lte=ay_end)

    # Fetch consultations with academic year filter
    consultations = await Consultations.filter(
        year_filter,
        patient_id=student_id,
        is_deleted=False,
    ...
    # Fetch lab test bookings with academic year filter
    lab_test_bookings = await LabTestBookings.filter(
        year_filter,
        patient_id=student_id,
        is_deleted=False,
    ...
    for consultation in consultations:
        ...
        appointments.append(AppointmentResponse(
            ...
            doctor_user_role=doctor_user_role,
            # slot_date=consultation.slot_date,
            slot_date=consultation.slot_date.isoformat(),
            slot_time=slot_time_str,
            consult_fee=float(consultation.consult_fee),
            booking_status=consultation.booking_status,
            type="Consultation",
...
# ===================================================================
# MODIFIED: GET SPECIALIST DECISIONS (with Academic Year Filter)
# ===================================================================
...
async def get_specialist_decisions(
    ...
    # Fetch specialist decisions with academic year filter
    ...
    for decision in decisions:
        decision_data.append({
            ...
            "specialist_role": decision.specialist_role.value
        })
...
#     now = datetime.now(timezone.utc)  # Use timezone.utc instead of pytz.UTC

#     # When combining date and time
#     slot_datetime_utc = datetime.combine(
#         slot_date_obj, 
#         slot_time_obj, 
#         tzinfo=timezone.utc
#     )
...
#     # Step 1: Delete expired pending slots automatically
#     # -----------------------------
#     expired_slots = await Consultations.filter(
#         booking_status="pending",
#         is_deleted=False
#     ).all()
...
#         doctor_id=payload["doctor_id"],
#         slot_date=slot_date_obj,
#         slot_time=slot_datetime_utc.replace(tzinfo=None),
#         is_deleted=False
#     ).first()
...
#         if existing_booking.booking_status == "confirmed":
#             raise HTTPException(status_code=400, detail="This slot is already booked.")
#         elif existing_booking.booking_status == "pending" and existing_booking.hold_expiry:
#             expiry_time = existing_booking.hold_expiry
#             if expiry_time.tzinfo is None:
#                 expiry_time = expiry_time.replace(tzinfo=timezone.utc)
...
#         consult_fee=payload["consult_fee"],
#         booking_status="pending",
#         hold_expiry=hold_expiry.replace(tzinfo=None),
#         created_by=parent_id,
#         created_user_role=current_user.get("user_role"),
#         created_role_type=current_user.get("role_type", "")
#     )
...
@router.post("/consultations-select-slot", response_model=StandardResponse)
async def select_consultation_slot(request: Request, current_user: dict = Depends(get_current_user)):
    """
    Allows a parent to temporarily reserve a consultation slot for 5 minutes before payment.
    Pending slots automatically expire after 5 minutes and get deleted.
    consult_id is auto-incremented by the database.
    ...
    try:
        ...
        payload = SlotSelectionRequest(**body)
        logger.info(f"Payload validated successfully: {payload}")
        ...
        # Validate doctor
        logger.info(f"Validating doctor_id: {payload.doctor_id}")
        doctor = await ConsultantTeam.get_or_none(id=payload.doctor_id, is_deleted=False)
        if not doctor:
            logger.error(f"Doctor not found: {payload.doctor_id}")
            raise HTTPException(status_code=404, detail="Doctor not found.")
        logger.info(f"Doctor validated: {doctor.id}")

        # Validate student exists
        logger.info(f"Validating student exists: patient_id={payload.patient_id}")
        student = await Students.get_or_none(id=payload.patient_id, is_deleted=False)
        if not student:
            logger.error(f"Student not found: {payload.patient_id}")
            raise HTTPException(status_code=404, detail="Student not found.")
        logger.info(f"Student validated: {student.id}")

        # Validate child relationship
        logger.info(f"Validating parent-child relationship: patient_id={payload.patient_id}, parent_id={parent_id}")
        is_child = await ParentChildren.filter(
            parent_id=parent_id,
            student_id=payload.patient_id,
            is_deleted=False
        ).exists()
        logger.info(f"Child validation result: {is_child}")
        
        # Note: Not enforcing parent-child relationship as per your commented code

        # -----------------------------
        # Normalize slot_time safely - Handle HH:MM format
        # -----------------------------
        try:
            # Try HH:MM:SS first
            slot_time_obj = datetime.strptime(payload.slot_time, "%H:%M:%S").time()
        except ValueError:
            try:
                # Try HH:MM (your frontend sends this)
                slot_time_obj = datetime.strptime(payload.slot_time, "%H:%M").time()
            except ValueError:
                logger.error(f"Invalid slot_time format: {payload.slot_time}")
                raise HTTPException(status_code=400, detail="Invalid slot_time format. Use HH:MM or HH:MM:SS.")

        # Validate and parse slot_date
        try:
            slot_date_obj = datetime.strptime(payload.slot_date, "%Y-%m-%d").date()
        except ValueError:
            logger.error(f"Invalid slot_date format: {payload.slot_date}")
            raise HTTPException(status_code=400, detail="Invalid slot_date format. Use YYYY-MM-DD.")

        logger.info(f"Parsed slot_time: {slot_time_obj}, slot_date: {slot_date_obj}")

        # Use IST timezone
        now = datetime.now(IST)
        now_utc = now.astimezone(pytz.UTC)
        logger.info(f"Current time IST: {now}, UTC: {now_utc}")
        
        # -----------------------------
        # Step 1: Delete expired pending slots using raw SQL
        # -----------------------------
        db = connections.get("default")
        
        logger.info("Deleting expired pending slots...")
        ...
        """
        await db.execute_query(delete_expired_query, [now_utc.replace(tzinfo=None)])
        logger.info("Expired slots deleted")

        # -----------------------------
        # Step 2: Check for existing booking using raw SQL
        # -----------------------------
        slot_time_str = slot_time_obj.strftime("%H:%M:%S")
        
        logger.info(f"Checking existing booking for doctor={payload.doctor_id}, date={slot_date_obj}, time={slot_time_str}")
        check_existing_query = f"""
            SELECT consult_id, booking_status, hold_expiry 
            FROM consultations 
            WHERE doctor_id = $1 
            AND slot_date = $2 
            AND slot_time = '{slot_time_str}'::time
            AND is_deleted = false
            LIMIT 1
        """
        ...
        result = await db.execute_query_dict(
            ...
            [payload.doctor_id, slot_date_obj]
        )
        logger.info(f"Existing booking check result: {result}")

        if result:
            existing = result[0]
            if existing["booking_status"] == "confirmed":
                raise HTTPException(status_code=400, detail="This slot is already booked.")
            elif existing["booking_status"] == "pending" and existing["hold_expiry"]:
                expiry_time = existing["hold_expiry"]
                if expiry_time.tzinfo is None:
                ...
                if expiry_time > now_utc:
                    ...
                    # Delete expired slot
                    logger.info(f"Deleting expired slot: {existing['consult_id']}")
                    delete_query = "DELETE FROM consultations WHERE consult_id = $1"
                    await db.execute_query(delete_query, [existing["consult_id"]])

        # -----------------------------
        # Step 3: Create pending booking using inline SQL value
        # -----------------------------
        hold_expiry_utc = now_utc + timedelta(minutes=5)
        hold_expiry_naive = hold_expiry_utc.replace(tzinfo=None)
        
        logger.info(f"Creating pending booking with hold_expiry: {hold_expiry_naive}")
        insert_query = f"""
            INSERT INTO consultations 
            (doctor_id, patient_id, slot_date, slot_time, consult_fee, booking_status, 
             hold_expiry, created_by, created_user_role, created_role_type, 
             created_at, updated_at, is_deleted)
            VALUES ($1, $2, $3, '{slot_time_str}'::time, $4, $5, $6, $7, $8, $9, $10, $11, $12)
            RETURNING consult_id
        """
        ...
        try:
            result = await db.execute_query_dict(
                ...
                [
                    ...
                    payload.patient_id,
                    slot_date_obj,
                    Decimal(str(payload.consult_fee)),
                    "pending",
                    hold_expiry_naive,
                    parent_id,
            ...
            if "tx_id" in error_msg and "not-null" in error_msg.lower():
                raise HTTPException(
                    status_code=500, 
                    detail="Database configuration error: tx_id cannot be null. Contact admin to run: ALTER TABLE consultations ALTER COLUMN tx_id DROP NOT NULL;"
                )
            elif "patient_id" in error_msg and "foreign key" in error_msg.lower():
                raise HTTPException(
                    status_code=400, 
                    detail=f"Invalid patient_id: {payload.patient_id}. Student does not exist in the system."
                )
            elif "doctor_id" in error_msg and "foreign key" in error_msg.lower():
                raise HTTPException(
                    status_code=400, 
                    detail=f"Invalid doctor_id: {payload.doctor_id}. Doctor does not exist in the system."
                )
            else:
                raise HTTPException(status_code=500, detail=f"Database integrity error: {error_msg}")
        
        consultation_id = result[0]["consult_id"]
        ...
        return StandardResponse.success_response(
            ...
            data={
                ...
                "hold_expires_at": hold_expiry_utc.isoformat()
            }
        )
