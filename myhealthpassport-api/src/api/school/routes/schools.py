...
async def school_details(school_id: int, current_user: dict = Depends(get_current_user)):
    ...
    students_strength = await SchoolStudents.filter(school_id=school_id, is_deleted=False).count()
    teachers_strength = await SchoolStaff.filter(school_id=school_id, is_deleted=False).count()
    try:
        school_logo = await get_new_url(school.school_logo)
    except:
        school_logo = ""
    ...
    school_details = {
        ...
        "school_name": school.school_name,
        "school_full_name": school.school_full_name,
        "school_students_strength": students_strength,
        "school_teachers_strength": teachers_strength,
        "school_logo": school_logo,
        "school_code": school.school_code,
        "registration_no": school.registration_no,
        "country_code": school.country_code,
...
# ):
#     """
#     Retrieves a paginated list of schools created in the specified academic year.
#     Requires user authentication.
#     """
...
#         resp = StandardResponse(
#             status=False,
#             message=f"{creator_role.value} is not allowed to view school records.",
#             data={},
#             errors={},
#         )
#         return JSONResponse(content=resp.__dict__, status_code=status.HTTP_403_FORBIDDEN)
...
#     # Build academic year filter for schools created_at
#     year_filter = build_academic_year_filter(academic_year)

#     # Filter schools by created_at within the academic year AND not deleted
#     schools_query = Schools.filter(year_filter, is_deleted=False)
#     total_schools = await schools_query.count()
...
#     if not db_schools:
#         response_payload = StandardResponse(
#             status=True,  # Changed to True as empty result is valid
#             message=f"No schools found for academic year {academic_year}.",
#             data={
#                 "schools_list": {
#                     "total": 0,
#                     "limit": limit,
...
#             errors={},
#         )
#         json_response = JSONResponse(content=response_payload.__dict__, status_code=status.HTTP_200_OK)
...
#         school_admin = await SchoolStaff.filter(
#             school=school,
...
#         # Count staff created in the same academic year for this school
#         total_staff = await SchoolStaff.filter(
#             year_filter,
#             school=school,
#             is_deleted=False
#         ).count()
        
#         # Count students enrolled in the same academic year for this school
#         total_students = await Students.filter(
#             year_filter,
#             school_students__school=school,
#             is_deleted=False
#         ).count()
...
#     response = StandardResponse(
#         status=True,
#         message=f"Schools retrieved successfully for academic year {academic_year}.",
#         data={
#             "schools_list": data_payload
#         },
#         errors={},
...
#     # Add academic year to response header
#     response_json = JSONResponse(content=response.__dict__, status_code=status.HTTP_200_OK)
#     response_json.headers["X-Academic-Year"] = academic_year
