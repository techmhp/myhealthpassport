      ...
      schoolDetails(schoolid)
        .then(res => {
          ...
          if (response.status === true) {
            setSchool_Info(response.data.school);
          }
        })
        .catch(err => {
          toastMessage(err.message || 'Unable to fetch the school info', 'error');
        });
    }
  }, [schoolid]);
      ...
      case 'View-All-Staff':
        return (
          <div className="bg-white rounded-lg">
            <ViewAllStaffMembers school_Info={school_Info} />
          </div>
        );
      default:
        return null;
  ...
  return (
    <>
      ...
      <div className="p-4 sm:p-6 md:p-[28px] md:px-12 lg:px-18">
        <div className="">
          <Breadcrumbs items={[{ name: 'View All Staff', href: '#', current: true }]} homeLabel="Roster" homeHref="/admin/roster/1" />
        </div>
