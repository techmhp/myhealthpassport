...
import { schoolDetails, studentDetails, downloadStudentReport } from '@/services/secureApis';
        ...
        <div className="px-2 sm:px-6 md:px-10 lg:px-14 grid gap-4 md:gap-8 lg:gap-13">
          ...
          <div className="w-full flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0">
            <Breadcrumbs
              items={[
                ...
                },
                {
                  name: student_details?.identity_details
                    ? `${student_details?.identity_details?.class_room}-${student_details?.identity_details?.section}`
                    : '',
                  href: `/admin/roster/${school.school_id}/class/${student_details?.identity_details?.class_room}${student_details?.identity_details?.section}`,
                  current: true,
                },
                {
              ...
              homeLabel="Roster"
              homeHref={`/admin/roster/${school.school_id}`}
            />
            <YearSelect />
          </div>
          ...
          <div className="fixed inset-0 bg-transparent bg-opacity-60 flex items-center justify-center z-[60]">
            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full mx-4">
              <div className="flex flex-col items-center">
                ...
                {/* Text Content */}
                <h3 className="text-xl font-semibold text-gray-800 mb-2">Preparing Your PDF</h3>
                <p className="text-sm text-gray-500 text-center mb-4">This may take a moment. Please don't close this window.</p>
              </div>
            </div>
          </div>
        )}
