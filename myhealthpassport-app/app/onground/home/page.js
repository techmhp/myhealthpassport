          ...
          <div className="mt-11 flex justify-between items-center">
            <div className="flex gap-5 items-center">
              ...
              <div className="flex items-center gap-2.5">
                <span className="text-[16px] font-medium leading-[25px]">{userInfo && userInfo.first_name ? formatString(`${userInfo.first_name} ${userInfo.last_name}`?.toLowerCase()) : <InlineSpinner />}</span>
                <div className="w-[24px] rotate-[-90deg] border border-black"></div>
                <span className="text-[16px] font-normal leading-[25px]">{userInfo.user_role ? formatString(userInfo.user_role?.toLowerCase()) : <InlineSpinner />}</span>
              </div>
            </div>
            <div>
              <span className="font-normal text-xs text-[#ACACAC]">{moment().format("dddd, Do MMMM YYYY h:mm a")}</span>
            </div>
          </div>
