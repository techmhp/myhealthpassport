            ...
            <div className={`grid grid-cols-1 gap-6 lg:grid-cols-${results?.data?.childrens.length > 4 ? 4 : results?.data?.childrens.length} mb-10`}>
              {results.status === true
                ? results.data.childrens.map((child, index) => (
                    <Link key={index} href={defaultRoute(child)} passHref>
                      <div className="flex flex-col gap-[20px] p-[15px] rounded-[10px] border border-[#DCDCDC] bg-white shadow-[0px_4px_4px_0px_#2537D733]">
