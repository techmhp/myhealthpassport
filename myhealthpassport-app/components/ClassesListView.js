...
const ClassesListView = ({ school = [] }) => {
  ...
  const baseUrlMap = {
    admin: `/admin/accounts/${school?.school_id}`,
    'school-admin': `/school-admin/accounts`,
  };
  ...
  const renderCategorySection = (categoryKey, displayName) => {
    ...
    return (
      <div className="mb-4" key={categoryKey}>
        <div className="mt-4 space-y-4 mx-4">{renderClasses(schoolData[categoryKey])}</div>
      </div>
    );
  };
