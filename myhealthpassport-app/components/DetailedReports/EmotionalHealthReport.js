...
const EmotionalHealthReport = ({ data, medical_data = {} }) => {
  ...
  const updateMedicalReport = async status => {
    ...
    const postObj = {
      ...
      medical_screening_statuses: {
        medical_officer_status_type: 'psychological_report_status',
        status: status,
  ...
  return (
    <div className="w-full flex flex-col gap-6 sm:gap-8 md:gap-10 pt-4 sm:pt-6 md:pt-8 px-4 sm:px-6 md:px-8 pb-4 sm:pb-6 md:pb-8 rounded-bl-lg rounded-br-lg border-t-0 border-r border-b border-l border-solid border-[#DCDCDC]">
      {Object.keys(results).length > 0 ? (
          ...
          <div className="flex flex-col mt-5 md:flex-row rounded-[10px] border border-white bg-[#EEFBF1] p-7">
            ...
            <div className="flex flex-col gap-[15px] mt-4 md:mt-0 md:pl-6 md:flex-1">
              ...
              <div className="leading-[26px]">
                {results.summary
                  ? stringToArray(results.summary).map((item, index) => (
                  ...
                  <div className="flex flex-col gap-3">
                    {results.recommendations ? (
                      typeof results.recommendations === 'object' ? (
                        results.recommendations.map((item, itemIndex) => (
                          <div key={itemIndex} className="flex items-start gap-3">
                            <StatusIcon status="star" />
                            <div className="leading-24px">
                              <span key={itemIndex} className="text-sm font-normal">
                                {item}
                              </span>
                            </div>
                          </div>
        ...
        <div className="text-center text-gray-500">There is no emotional health report.</div>
      )}
      {remarks && (
        ...
        <>
          <div className="mb-5 mt-5 flex justify-center items-center gap-5">
            ...
            <button
              ...
              className="rounded-[5px] cursor-pointer bg-indigo-500 px-5 py-2 text-sm font-normal text-white shadow-xs hover:bg-indigo-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
...
export default EmotionalHealthReport;
