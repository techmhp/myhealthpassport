...
import { createExpert } from '@/services/secureApis';
...
export default function AddExpertForm() {
  ...
  const handleSlotChange = (day, slot) => {
    setAvailability(prev => {
      ...
      if (existingDay) {
        ...
        // If no slots left, remove the day completely
  ...
  const formFields = [
    ...
    },
    {
      ...
      fields: [
        // Note: clinic-name and clinic-email are not in the schema, so excluded
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        ...
        const payload = {
          ...
          languages_spoken: formData.languages_spoken ? formData.languages_spoken.split(',').map(lang => lang.trim()) : [],
        ...
        const response = await createExpert(JSON.stringify(payload));
      ...
      <form onSubmit={handleSubmit} method="POST" encType="multipart/form-data">
        <div className="space-y-12">
          {formFields.map(formField => (
            <div key={formField.section} className="border-b border-gray-200 pb-8">
              <h2 className="text-lg font-semibold text-gray-900 mb-6">{formField.section}</h2>
