...
import { createEyePrescription, getMedicalReportStatus } from '@/services/secureApis';
...
export default function OphthalmologistReport() {
  ...
  //   'Colour vision Test Normal',
  //   'Colour vision Test Abnormal',
  ...
  const handleAddAdditionalFinding = () => {
    if (additionalFindingRemarks.trim()) {
      const newFinding = {
        id: Date.now(),
        remarks: additionalFindingRemarks.trim(),
      };
      setAdditionalFindings([...additionalFindings, newFinding]);
      setAdditionalFindingRemarks('');
    }
  ...
  // Additional Findings Edit handlers
  // const handleEditAdditionalFinding = index => {
  //   const finding = additionalFindings[index];
  //   setEditingFinding(index);
  //   setTempEditNote(finding.note);
  //   setTempEditRemarks(finding.remarks);
  // };
  ...
  // const handleSaveAdditionalFinding = index => {
  //   if (tempEditNote) {
  //     const updated = [...additionalFindings];
  //     updated[index] = {
  //       ...updated[index],
  //       note: tempEditNote,
  //       remarks: tempEditRemarks,
  //     };
  //     setAdditionalFindings(updated);
  //   }
  ...
  const handleSaveAdditionalFinding = index => {
    if (tempEditRemarks.trim()) {
      const updated = [...additionalFindings];
      updated[index] = {
        ...updated[index],
        remarks: tempEditRemarks.trim(),
      };
      setAdditionalFindings(updated);
    }
    setEditingFinding(null);
  ...
  // const handleCancelEditAdditionalFinding = () => {
  //   setEditingFinding(null);
  //   setTempEditNote('');
  //   setTempEditRemarks('');
  // };
  ...
  // Recommendation Edit Handlers
  const handleEditRecommendation = id => {
    const recommendation = recommendations.find(r => r.id === id);
    setEditingRecommendation(id);
    setTempEditValue(recommendation.text);
  };

  const handleSaveRecommendation = id => {
    if (tempEditValue.trim()) {
      const updated = recommendations.map(rec => (rec.id === id ? { ...rec, text: tempEditValue.trim() } : rec));
  ...
  const buildRequestPayload = () => {
    ...
    const additionalFindingsArray = additionalFindings.map(finding => finding.remarks);
    ...
    return {
      ...
      additional_findings: additionalFindingsArray,
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await createEyePrescription(JSON.stringify(requestBody));

      if (response.status === true) {
        toastMessage(response.message || 'Eye screening saved successfully!', 'success');
        router.back();
      } else {
        toastMessage(response.message || 'Failed to save eye screening.', 'error');
      }
    } catch (err) {
  ...
  return (
    <div className="w-full pt-[35px] pr-[34px] pb-[60px] pl-[34px] flex flex-col gap-10">
      ...
      <div className="flex flex-col gap-10">
            ...
            <div className="flex right">
              ...
              <div className="flex-col p-2">
                ...
                <span className="leading-[10px] text-[14px]">
                  {userInfo?.education || 'MBBS'} | {userInfo?.specialty || 'EYE SPECIALIST'}
                </span>
              </div>
            </div>
          </div>
                    ...
                    {editingPatientConcern === index ? (
                      <>
                        <div className="flex-1 flex gap-3 items-center">
                          ...
                          <input
                            ...
                            autoFocus
                            onKeyDown={e => {
                              if (e.key === 'Enter') handleSavePatientConcern(index);
                              if (e.key === 'Escape') handleCancelEditPatientConcern();
                            }}
                          />
                        </div>
                        <div className="flex gap-2 items-center ml-3">
                          <button
                            type="button"
                            onClick={() => handleSavePatientConcern(index)}
                          ...
                          <button
                            type="button"
                            onClick={handleCancelEditPatientConcern}
                      ...
                      <>
                        ...
                        <div className="flex gap-3 items-center">
                          ...
                          <button type="button" className="cursor-pointer focus:outline-none" title="Edit" onClick={() => handleEditPatientConcern(index)}>
        ...
        <div className="w-full flex flex-col gap-5">
          <div className="flex justify-between">
            <h1 className="w-[35%] font-semibold text-[14px] leading-[100%] tracking-[0%]">Vision Test Results</h1>
                  ...
                  {editingFinding === index ? (
                    <>
                      <div className="flex-1 flex gap-3 items-center">
                        ...
                        <div className="flex-1 flex gap-2">
                          ...
                          <input
                            ...
                            onKeyDown={e => {
                              if (e.key === 'Enter') handleSaveAdditionalFinding(index);
                              if (e.key === 'Escape') handleCancelEditAdditionalFinding();
                            }}
                          />
                        </div>
                      </div>
                      <div className="flex gap-2 items-center ml-3">
                        <button
                          type="button"
                          onClick={() => handleSaveAdditionalFinding(index)}
                          className="text-green-600 hover:text-green-700 focus:outline-none"
                          title="Save"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="size-5">
                        ...
                        <button
                          type="button"
                          onClick={handleCancelEditAdditionalFinding}
                          className="text-red-600 hover:text-red-700 focus:outline-none"
                          title="Cancel"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="size-5">
                    ...
                    <>
                      <div className="flex gap-3 items-center">
                        ...
                        <p className="mb-0 font-normal text-sm leading-6">{finding.remarks}</p>
                      </div>
                      <div className="flex gap-3 items-center">
                        ...
                        <button type="button" className="cursor-pointer focus:outline-none" title="Edit" onClick={() => handleEditAdditionalFinding(index)}>
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="#5389FF" className="size-5">
                            <path d="m5.433 13.917 1.262-3.155A4 4 0 0 1 7.58 9.42l6.92-6.918a2.121 2.121 0 0 1 3 3l-6.92 6.918c-.383.383-.84.685-1.343.886l-3.154 1.262a.5.5 0 0 1-.65-.65Z" />
                            <path d="M3.5 5.75c0-.69.56-1.25 1.25-1.25H10A.75.75 0 0 0 10 3H4.75A2.75 2.75 0 0 0 2 5.75v9.5A2.75 2.75 0 0 0 4.75 18h9.5A2.75 2.75 0 0 0 17 15.25V10a.75.75 0 0 0-1.5 0v5.25c0 .69-.56 1.25-1.25 1.25h-9.5c-.69 0-1.25-.56-1.25-1.25v-9.5Z" />
                          </svg>
          ...
          <div className="flex justify-between">
            ...
            <div className="flex-1 flex flex-col gap-7">
                      ...
                      <>
                        <div className="flex-1 flex gap-3 items-center">
                          ...
                          <input
                            ...
                            autoFocus
                            onKeyDown={e => {
                              if (e.key === 'Enter') handleSaveRecommendation(recommendation.id);
                              if (e.key === 'Escape') handleCancelEditRecommendation();
                            }}
                          />
                        </div>
                        <div className="flex gap-2 items-center ml-3">
              ...
              <div className="w-full flex gap-4 items-center justify-between">
                ...
                />
                <button onClick={handleAddRecommendation} className="w-[22px] h-[22px] hover:opacity-70">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="#5389FF" className="size-5">
                    <path
                      fillRule="evenodd"
                      d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm.75-11.25a.75.75 0 0 0-1.5 0v2.5h-2.5a.75.75 0 0 0 0 1.5h2.5v2.5a.75.75 0 0 0 1.5 0v-2.5h2.5a.75.75 0 0 0 0-1.5h-2.5v-2.5Z"
