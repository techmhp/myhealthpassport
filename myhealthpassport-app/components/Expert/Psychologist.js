...
import { createPsychologistPrescription, getMedicalReportStatus } from '@/services/secureApis';
import { toastMessage } from '@/helpers/utilities';
import { useRouter, useParams } from 'next/navigation';
import InlineSpinner from '../UI/InlineSpinner';

export default function PsychologistReport() {
  const router = useRouter();
  const { id } = useParams();

  // User and Report State
  const [userInfo, setUserInfo] = useState(null);
  const [medicalOfficerPsychologyReport, setMedicalOfficerPsychologyReport] = useState({});
  ...
  const [findingAdditionalNotes, setFindingAdditionalNotes] = useState('');
  ...
  const fetchMedicalReportStatus = () => {
    getMedicalReportStatus(id)
      .then(res => {
        ...
        const psychologyReport = response.data.medical_screening_statuses?.find(item => item.medical_officer_status_type === 'psychology_screening_status');
        setMedicalOfficerPsychologyReport(psychologyReport || {});
      })
      .catch(err => {
  ...
  const handleRemovePatientConcern = index => {
    const removed = patientConcerns[index];
  ...
  const handleAddFinding = () => {
    if (selectedFinding) {
      const findingText = findingRemarks ? `${selectedFinding}: ${findingRemarks}` : selectedFinding;
      const newFinding = {
        id: Date.now(),
        text: findingText,
      };
  ...
  const handleRemoveFinding = indexToRemove => {
    const removed = findings[indexToRemove];
    setFindings(findings.filter((_, index) => index !== indexToRemove));
  };
  ...
  const handleRemoveRecommendation = id => {
    const removed = recommendations.find(rec => rec.id === id);
  ...
  const buildRequestPayload = () => {
    const payload = {
      ...
      consultant_user_id: parseInt(userInfo?.user_id),
      patient_concern: patientConcerns,
      findings: findings.map(finding => finding.text),
      treatment_recommendations: recommendations.map(rec => rec.text),
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await createPsychologistPrescription(JSON.stringify(requestBody));

      if (response.status === true) {
        const successMessage = 'Psychology screening saved successfully!';
        toastMessage(response.message || successMessage, 'success');
        router.back();
      } else {
        const errorMessage = 'Failed to save psychology screening.';
  ...
  return (
    <div className="w-full pt-[35px] pr-[34px] pb-[60px] pl-[34px] flex flex-col gap-10">
      ...
      {medicalOfficerPsychologyReport?.remarks && (
        <div className="w-full space-y-5 sm:space-y-7.5 pl-5 py-5 bg-[#FFF3E5] rounded-[8px]">
          <div className="flex gap-5">
            ...
            <span className="text-red-500 text-sm font-normal">{medicalOfficerPsychologyReport.remarks}</span>
          </div>
        </div>
      )}
        ...
        <div className="w-full space-y-[11px]">
          <div className="flex justify-between">
            <div className="flex left">
              ...
              <span className="leading-[24px] p-4 flex-col font-semibold text-[14px]">{userInfo?.clinic_name || 'Psychologist Clinic'}</span>
            </div>
            <div className="flex right">
              ...
              <div className="flex-col p-2">
                ...
                <span className="leading-[10px] text-[14px]">
                  {userInfo?.education || 'MBBS'} | {userInfo?.specialty || 'Psychologist'}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="flex-1 flex flex-col gap-7">
              ...
              {findings.map((finding, index) => (
                <div key={finding.id} className="w-full flex items-center justify-between">
                  <div className="flex gap-3 items-center">
                    <div className="rounded-full w-[10px] h-[10px] bg-[#5389FF]" />
                    <p className="mb-0 font-normal text-sm leading-6">{finding.text}</p>
                  </div>
                  <div className="flex gap-3 items-center">
                    <button type="button" onClick={() => handleRemoveFinding(index)} className="focus:outline-none">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
              ...
              <div className="w-full flex gap-4 items-center justify-between">
                <select
                  className="appearance-none w-full border border-[#D5D9E2] rounded-[8px] px-4 py-[10px] text-sm bg-white"
                  value={selectedFinding}
                  onChange={e => setSelectedFinding(e.target.value)}
                >
                  <option value="">Select</option>
                  {findingOptions.map((option, idx) => (
                ...
                <input
                  ...
                  className="w-full border border-[#D5D9E2] rounded-[8px] px-4 py-[10px] text-sm bg-white"
                />
                <button type="button" onClick={handleAddFinding} className="w-[22px] h-[22px] flex items-center justify-center focus:outline-none">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="#5389FF" className="size-5 cursor-pointer">
                    <path
                      fillRule="evenodd"
                      d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm.75-11.25a.75.75 0 0 0-1.5 0v2.5h-2.5a.75.75 0 0 0 0 1.5h2.5v2.5a.75.75 0 0 0 1.5 0v-2.5h2.5a.75.75 0 0 0 0-1.5h-2.5v-2.5Z"
              ...
              {/* Additional Notes for Findings */}
              {/* <div className="w-full flex flex-col gap-5">
                <p className="font-medium text-sm leading-6 mb-0">Additional Notes</p>
                <textarea
                  placeholder="Start Typing..."
                  value={findingAdditionalNotes}
                  onChange={e => setFindingAdditionalNotes(e.target.value)}
                  className="w-full border border-gray-300 rounded-[8px] px-4 py-[10px] text-sm leading-6 focus:outline-none resize-none bg-white"
                  rows="6"
                />
              </div> */}
            </div>
            ...
            <div className="flex-1 flex flex-col gap-7">
              ...
              <div className="w-full flex gap-4 items-center justify-between">
                <input
                  ...
                  className="w-full border border-[#D5D9E2] rounded-[8px] px-4 py-[10px] text-sm leading-6 font-normal text-gray-900 placeholder-gray-400 focus:outline-none bg-white"
                />
      ...
      <div className="mb-[50px] flex justify-center items-center gap-5">
        ...
        <button
          type="button"
          className="rounded-[5px] bg-indigo-500 h-[37px] px-5 py-2 text-sm font-normal text-white shadow-xs hover:bg-indigo-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed"
          onClick={handleSave}
          disabled={saving}
        >
          {saving ? (
