...
import { getPediatricianPrescription } from '@/services/secureApis';
import { toastMessage } from '@/helpers/utilities';
import { useParams } from 'next/navigation';
import InlineSpinner from '@/components/UI/InlineSpinner';

const PediatricianPrescription = ({ academicYear = null }) => {
  const { id } = useParams();

  // State Management
  const [pediatricianData, setPediatricianData] = useState(null);
  ...
  useEffect(() => {
    const fetchPediatricianData = async () => {
      try {
        ...
        const response = await getPediatricianPrescription(id, academicYear);
        const responseData = JSON.parse(response);
        if (responseData.status === true && responseData.data) {
          setPediatricianData(responseData.data);
        } else if (responseData.status === false) {
          setError(responseData.message || 'Failed to fetch pediatrician data');
          // toastMessage(responseData.message || 'Failed to fetch pediatrician data', 'error');
        }
      } catch (err) {
  ...
  if (error) {
    return (
      <div className="w-full pt-[35px] pr-[34px] pb-[25px] pl-[34px] flex justify-center items-center">
    ...
    return (
      <div className="w-full pt-[35px] pr-[34px] pb-[60px] pl-[34px] flex justify-center items-center">
        <div className="text-center text-gray-500">
          <p>No pediatrician data found</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="w-full space-y-[11px]">
          <div className="flex justify-between">
            <div className="flex left">
              ...
              <span className="leading-[24px] p-4 flex-col font-semibold text-[14px]">{pediatricianData.clinic_name || 'Pediatric Clinic'}</span>
            </div>
            <div className="flex right">
              ...
              <div className="flex-col p-2">
                <span className="leading-[10px] font-semibold text-[14px]">{pediatricianData?.full_name || 'Student Name'}</span>
                <br />
                <span className="leading-[10px] text-[14px]">
                  {pediatricianData?.age || 'Age'} | {pediatricianData?.gender || 'Gender'}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="w-full space-y-[11px]">
          <div className="flex pb-[11px] justify-between">
            ...
            <div className="w-[60%] flex flex-col gap-2">
              {pediatricianData.patient_concern && pediatricianData.patient_concern.length > 0 ? (
                pediatricianData.patient_concern.map((concern, index) => (
        ...
        <div className="w-full space-y-[11px]">
          <div className="flex pb-[11px] justify-between">
            ...
            <div className="w-[60%] flex flex-col gap-4">
              {pediatricianData.findings && pediatricianData.findings.length > 0 ? (
                pediatricianData.findings.map((finding, index) => (
        ...
        <div className="w-full space-y-[11px]">
          <div className="flex pb-[11px] justify-between">
            ...
            <div className="w-[60%] flex flex-col gap-4">
              {pediatricianData.treatment_recommendations && pediatricianData.treatment_recommendations.length > 0 ? (
                pediatricianData.treatment_recommendations.map((recommendation, index) => (
                  <div key={index} className="flex items-start gap-3">
        ...
        <div className="w-full space-y-[11px]">
          <div className="flex pb-[11px] justify-between">
            ...
            <div className="w-[60%] flex flex-col gap-4">
              <p className="mb-0 font-normal text-sm leading-6">{pediatricianData.next_followup || ''}</p>
            </div>
          </div>
          <hr className="border-t border-[#B7B7B7]" />
        </div>
          ...
          <div className="flex pb-[11px]">
            <div className="w-full">
              <div className="flex-col p-2 float-right">
                ...
                <span className="leading-[24px] font-semibold text-[14px]">Dr. {pediatricianData.consultant_full_name || 'Doctor Name'}</span>
                <br />
                <span className="leading-[24px] text-[14px]">
                  {pediatricianData.education || 'MBBS'} | {pediatricianData.specialty || 'Pediatrician'}
                </span>
              </div>
            </div>
          </div>
...
export default PediatricianPrescription;
