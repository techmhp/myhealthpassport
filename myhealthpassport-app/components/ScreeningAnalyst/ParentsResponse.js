...
import { nutritionalQuestions, developmentalQuestions } from '@/services/secureApis';
...
export default function NutritionalQuestionnaire() {
  ...
  const getNutritionalQuestionnare = async studentId => {
    try {
      const rawResponse = await nutritionalQuestions(studentId);
  ...
  const getDevelopmentalQuestionnare = async studentId => {
    try {
      const rawResponse = await developmentalQuestions(studentId);
      const result = JSON.parse(rawResponse);
      // if (result.status !== true) {
      //     toastMessage(result.message || `Failed to fetch data: ${rawResponse}`, 'error');
      //     return;
                ...
                results.data.questions.length > 0 ? (
                  results.data.questions.map((question, index) => (
                    <div key={index} className="flex flex-col">
                      ...
                      <p className="text-blue-500 text-medium font-semibold">Ans: {question.answer}</p>
                    </div>
                  ))
                ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-x-[20px] gap-y-[40px] sm:gap-x-[25px] sm:gap-y-[50px] md:gap-x-[25px] md:gap-y-[50px] lg:gap-x-[30px] lg:gap-y-[70px]">
              {results.status === true ? (
                results.data?.questions.length > 0 ? (
                  results.data.questions.map((question, index) => (
                    <div key={index} className="flex flex-col">
                      <p className="font-inter font-normal text-sm leading-6 tracking-normal text-[#000000] mb-2">{`${index + 1}. ${
                ...
                <div className="col-span-full text-center text-red-500">{results.message || 'Failed to load questions'}</div>
              )}
            </div>
          </div>
        );
