...
import { createTeacher } from '@/services/secureApis';
...
export default function AddTeacherForm() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const response = await createTeacher(JSON.stringify(formData));
      if (response.status === true) {
        const successMessage = response.message || 'Teacher created successfully';
        toastMessage(successMessage, 'success'); // Success toast
        router.back();
      } else {
        const errorMessage = response.message || 'An error occurred while creating teacher';
