...
import { updateProfile } from '@/services/secureApis';
import { formatString } from '@/helpers/utilities';
import { Plus } from 'lucide-react';
import { toastMessage } from '@/helpers/utilities';

export default function UpdateStaffMemberForm({ profile }) {
  ...
  useEffect(() => {
    ...
    const { profile_image, ...restOfData } = profile;
  ...
  const handleSubmit = async e => {
    ...
    try {
      const response = await updateProfile(id, JSON.stringify(formData));
        ...
        <div className="mb-7 flex flex-col sm:flex-row sm:items-center gap-y-3 gap-x-4 md:gap-x-6 w-full">
          ...
          >
            {preview || profile?.profile_image ? (
              <img src={preview !== null ? preview : profile?.profile_image} alt="Profile" className="w-full h-full object-cover rounded-full" />
            ) : (
          ...
          <div className="flex flex-col">
            ...
            <div className="flex flex-row gap-2">
              <p className="text-sm">
                Age: <strong>{formData.age ? formData.age : '---'}</strong>
              </p>
              <div className="mx-1 sm:mx-2 h-5 border-l border-[#000000]" />
              <p className="text-sm">
                Gender: <strong>{formData.gender ? formData.gender : '---'}</strong>
              </p>
              <div className="mx-1 sm:mx-2 h-5 border-l border-[#000000]" />
              <p className="text-sm">
                <strong>{formatString(formData?.user_role?.toLowerCase()) || '---'}</strong>
              </p>
            </div>
          </div>
        </div>
                    ...
                    <div className="mt-2">
                      <div className="relative">
                        <select
                          ...
                          value={formData?.spoken_languages ? formData.spoken_languages : ''}
                          onChange={handleChange}
                          className="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6 appearance-none"
                        >
                      ...
                      {errors.spoken_languages && <p className="mt-1 text-xs text-red-500">{errors.spoken_languages}</p>}
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div className="max-w-sm">
                    ...
                    <div className="mt-2">
                      <DatePicker
                        ...
                        defaultValue={profile?.dob !== null ? dayjs(profile?.dob) : ''}
                  ...
                  <div className="max-w-sm">
                    ...
                    <div className="mt-2">
                      <input
                        ...
                        type="text"
                        defaultValue={formData.country_calling_code}
                        onChange={handleChange}
                        className="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
                      />
                    </div>
