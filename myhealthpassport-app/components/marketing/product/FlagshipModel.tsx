...
const FlagshipModel = () => {
  const pillars = [
    ...
    { icon: Eye, label: "Vision", desc: "Eye screening", color: "text-coral", bg: "bg-orange-50" },
    { icon: Shield, label: "Dental", desc: "Oral health", color: "text-lavender", bg: "bg-violet-50" },
  ];
    ...
    <div className="space-y-10">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {pillars.map((p, i) => (
            <div key={i} className={`flex flex-col items-center gap-2 p-5 rounded-2xl ${p.bg} border border-border/50 text-center`}>
      ...
      {/* Student Curriculum — Psychology + Nutrition */}
      <div>
        ...
        <div className="space-y-4">
          {studentStages.map((stage, i) => (
            <div key={i} className={`rounded-2xl border ${stage.border} ${stage.bg} overflow-hidden`}>
              {/* Stage header */}
              <div className="px-5 pt-5 pb-3 flex items-baseline gap-3 flex-wrap">
